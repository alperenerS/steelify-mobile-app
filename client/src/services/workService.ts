...
export const getWorks = async (token: string): Promise<{ works: Work[], workSteps: WorkSteps[], workProducts: WorkProducts[] }> => {
  ...
  if (netInfo.isConnected && netInfo.isInternetReachable) {
    try {
      const response = await axios.get(`${BASE_URL}/api/worksbyid`, {
        headers: {
      ...
      if (response.status === 200) {
        ...
        const workStepsResponse = await axios.post(`${BASE_URL}/api/worksteps`, { ids });
        ...
        if (workStepsResponse.status === 200) {
          ...
          const workProductsResponse = await axios.post(`${BASE_URL}/api/workproducts`, { ids: workIds });
...
export const getWorkById = async (workId: number): Promise<{ workInfo: WorkInfo[] }> => {
  ...
  if (netInfo.isConnected && netInfo.isInternetReachable) {
    try {
      const response = await axios.get(`${BASE_URL}/api/work/${workId}`);
...
export const getForm = async (productId: number, vendorId: number): Promise<{ form: Form[] }> => {
  ...
  if (netInfo.isConnected && netInfo.isInternetReachable) {
    try {
      const response = await axios.post(`${BASE_URL}/api/formids`, {
...
export const postQualityControl = async (formId: number, workId: number): Promise<{ qualitycontrol: QualityControl[] }> => {
  ...
  if (netInfo.isConnected && netInfo.isInternetReachable) {
    try {
      const response = await axios.post(`${BASE_URL}/api/qualitycontrol`, {
        form_id: formId,
        work_id: workId
      });
